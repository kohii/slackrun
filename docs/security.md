...
`slackrun start` opens a UNIX-domain socket for `runs`, `kill`, and
`prepare-restart`. The last command atomically closes admission on an idle
dispatcher and asks the daemon to exit cleanly, so an operator can restart it
without racing a new job. There is no TCP listener.
...
| The child-facing help (`slackrun_help` stdin part, `ChildUsage`) does **not** advertise admin commands. | `internal/clidoc/usage.go` lists them only in `MainUsage`. |
...
strips `SLACKRUN_ADMIN_SOCKET` from the child's env so an untargeted admin
command from a child doesn't just work "by accident", but a child that
resolves the default socket path can still reach the API, including stopping an
idle daemon with `prepare-restart`. If that matters for your rule set, run
those children under a different UID (e.g.
